...
当前尤其不能混淆以下状态：`dxaApproximate()` 只是按邻居数寻找疑似缺陷核心，未生成 Burgers 矢量与位错线；当前“近似结构分类”不是 CNA；Create bonds 产生键对但尚未渲染。Color coding 曾经选择非 Position 属性时仍由 GPU 按坐标着色；本轮已修正该路径并增加 GPU 像素回归，但渐变精度、逐节点设置、图例和全帧范围仍未完成。Pipeline 已开始迁移：主程序现在把真实 `Modifier` 参数存入 `PipelineGraph` 节点并由该图执行；节点级缓存和完整结果数据对象检查器尚未完成。以上项目应在可验收记录中显式标为“部分/有误”，不得出现在“已对齐”汇总里。
...
- **下一步：** 给 CNA 增加 HCP/icosahedral fixtures 和参数限制提示；随后实现 Assign color、结果检查器与颜色节点独立状态，再继续基础 Modification 和更多分析算法。
