...
- **2026-09-25：P2 波次（Affine transformation / Expand selection / Select type）。** Affine 补 Enter rotation 旋转预设模态、约化坐标平移、仅选中变换与只读晶胞联动显示；Expand selection 升级为 OVITO 四种扩展模式（cutoff / N nearest / bonded / same molecule）+ 迭代次数 + OVITO 同文错误；Select type 改为 Name|Id 多选复选框表。新增 13 项回归（旋转/约化坐标/仅选中、N nearest 与 cutoff 发散样本、双步迭代、键拓扑与同分子、多类型并集计数），构建/测试/无头截图全绿，GUI 三面板现场比对通过。遗留：空选区错误呈现方式与 OVITO 不同（模态弹窗 vs 面板内红字）。
- **2026-09-25：N2 波次（Displacement vectors）。** 由灰显占位转为真实修饰器：参考帧绝对/相对解析（带越界钳制）、逐节点缓存的原始帧读取、原子索引匹配 + 标识符哈希回退、晶胞映射三态、逐轴最小镜像；发布 Displacement 矢量与 Displacement Magnitude 标量属性供下游消费。热振动解析基准 ±3e-4 内一致；面板按实测 OVITO 布局。遗留：外部文件参考源、箭头可视元素、参考取上游修饰结果（当前取原始帧）。
- **2026-09-25：P3 波次（Create bonds / Color coding / 目录册）。** Create bonds 面板按 OVITO 重排并新增真实 Lower cutoff 最小距离过滤与默认 cutoff 3.2；Color coding 重排为 OVITO 顺序并新增渐变条预览、Adjust range/(all frames)/Reverse range 与 Rainbow 默认；目录册各卡片组内字母序（AtomX 额外项穿插）。Flat shading 按诚实原则省略；Reverse range 以渐变反转等价实现并有工具提示。构建/测试/无头截图全绿，GUI 现场比对通过。
- **2026-09-25：N3 波次（Freeze property / Unwrap trajectories）。** Freeze 在参考帧快照上游粒子属性并在其余帧还原（索引配对 + Particle Identifier 回退，计数不匹配报 OVITO 同文错误），面板仅列出可冻结上游属性并给出诚实空态；Unwrap 与 OVITO 一致无参数，经节点级最小图像累加器沿周期轴连续展开位置。引擎新增可选逐节点跨帧状态回调。九项新回归；构建/测试/无头截图全绿；GUI 现场验收通过。本轮 check-improve 共 6 个波次、19 个本地提交，账面实现/部分条目从 34 上升到 45，其中 15 项引擎达到 pass 门槛。
- **2026-09-25：P4 波次（用户验收反馈：界面紧凑度与拖拽打开）。** 工具栏改为 14 个分组图标按钮（Segoe MDL2 运行时合并字体 + 逐字形回退文字），行高 48→34；时间轴改为右对齐紧凑传输簇 + 帧号步进器；右栏新增 Pipelines 选择器、四图标操作行与节点列表竖排图标条（作用于选中节点）；新增 shell 拖拽打开（DragAcceptFiles + WM_DROPFILES 走既有加载路径，多文件与解析失败有明确反馈）。伪造 HDROP 注入端到端验证；构建/测试全绿，UI 交互回归按记录名全部通过。
- **2026-09-25：P6 波次（用户验收反馈：默认配色与半径）。** 新增 64 元素文献数据表（Jmol/CPK 颜色 + Cordero 2008 共价半径 + Bondi 1964 VDW），类型名匹配元素时默认外观改为元素默认（未知类型保留调色板），用户编辑仍优先；粒子外观面板显示带出处的元素默认半径；渲染器/着色器/相机适配解析该默认。实测 test.xyz 的 Cu/Ni 颜色与半径正确。构建/测试全绿。
