...
## 2026-09-25 P5 波次验证（用户验收反馈三项修正）

- 图标行替代文字 Tab：[Pipeline][Render][Analysis][System] 标签栏移除，四个 Segoe MDL2 图标为分区切换器（激活态高亮、带提示、panel.tab.* 记录名保留），Pipelines: 选择器与 Add modification... 按 OVITO 顺序排列；icon-tab 点击回归（0→1→3→2→0）。
