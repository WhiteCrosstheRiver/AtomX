...
- The export workflow integration test runs a real `App`/ImGui frame loop, records stable control IDs and rectangles, clicks Add modification and the RDF catalog entry, edits RDF parameters by keyboard, and verifies the recomputed table. It also uses visible Undo/Redo, selects and copies the node, changes only the copy's bin count, and deletes the copy while checking the original remains unchanged. Recorded interactive controls must fit inside the application bounds; this caught and fixed Pipeline action buttons clipped beyond the stack child and an oversized RDF input. This tests ImGui widget/event behavior without claiming Windows-level desktop input or accessibility-tree coverage.
