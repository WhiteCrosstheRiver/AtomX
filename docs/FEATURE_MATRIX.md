...
- Bond length and bond angle distributions are pipeline nodes with configurable histogram bins and per-node absolute-count, relative-frequency, or probability-density output. Probability density is normalized by the physical bin width (length units or degrees). They measure explicit bonds including periodic image translations through one shared geometry validator, publish count and range globals, and emit data tables. Invalid endpoints, zero-length bonds, non-finite geometry, and image shifts on non-periodic axes fail at the owning node. Bond order analysis remains unavailable.
