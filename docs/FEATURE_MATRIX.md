...
- The export workflow integration test now also runs a real `App`/ImGui frame loop, records stable control IDs and rectangles, clicks Add modification and the RDF catalog entry through ImGui pointer events, then edits the RDF bin-count control by keyboard and verifies the recomputed table. This tests application widget/event behavior without claiming Windows-level desktop input or accessibility-tree coverage.
