...
| Preview mode / Adjust view | 部分 | Fit all、Fit selected 按可见细胞、粒子半径/形状边界取景；正交投影按实际视口纵横比拟合，透视投影计算使八角点留边的最小安全相机距离；没有输出安全框 |
...
| Particles / Simulation cell / Global attributes | 部分 | 虚拟化粒子/属性表、晶胞三向量/原点/PBC、键和全局属性；检查器可切换最终管线结果或按需异步重算所选节点输出，节点检查按管线代数产生临时结果并在管线/帧变化时取消或丢弃；单帧检查可能暂时增加一份数据集内存占用；Edit simulation cell 默认只改晶胞 |
...
| Intel / NVIDIA / AMD | 部分 | 通用 D3D11 feature level 11.0 路径，Intel 与 NVIDIA 在本机实测；AMD 未实测。DXGI 适配器按 LUID 身份去重并标记当前实际使用设备，名称相同但 LUID 不同的 GPU 分开显示 |
| 几亿原子 | 未达到完整目标 | 流式扫描 + 有界采样；没有几亿原子全量显存驻留 / 全精度交互的验证 |

## Pipeline / analysis status, 2026-09-23

- Color coding is stored per pipeline node and writes the selected Position, scalar, or vector-component values into the evaluated dataset. Property extraction checks cancellation while copying large arrays. Automatic/symmetric/manual range, exact cancellable range scanning across all supported input formats (up to 2 million atoms per frame; static structures are scanned as one frame), discrete mapping, inversion, selected-only coloring, and keep-selection are implemented. Current-frame and trajectory-wide range scans support vector components. The viewport and active-view PNG show the selected property and numeric range. The D3D11 viewport, legend and PNG now sample one shared 256-step table; Magma, Viridis and Plasma use Matplotlib's published 256-color tables (8-bit RGB), while other gradients retain AtomX's existing formulas sampled into the common table.
- Coloring parameters are edited on their owning Pipeline node; the System panel reports the effective coloring configuration without a second set of unsynchronized global editors.
...
- Replicating a periodic bond-bearing cell is regression-tested across a boundary; every remapped endpoint and image shift preserves the original Cartesian bond vector in the enlarged cell.
- Undo and redo exchange complete modifier-node snapshots without rewriting node IDs or color-coding trajectory-range settings; mouse-drag edits coalesce into one starting snapshot per press gesture, while keyboard and discrete edits remain independent.
...
- RDF numerical regression compares orthogonal FCC against a rigidly rotated, triclinic-cell instance and verifies identical coordination and all radial bins; partially periodic slabs are explicitly excluded from 3D RDF.
...
- Bond length and bond angle distributions are pipeline nodes with configurable histogram bins and per-node absolute-count, relative-frequency, or probability-density output. Probability density is normalized by the physical bin width (length units or degrees). They require full data, measure explicit bonds including periodic image translations through one shared geometry validator, publish count and range globals, and emit data tables. Cancellation is checked while building, enumerating, and binning measurements. Invalid endpoints, zero-length bonds, non-finite geometry, and image shifts on non-periodic axes fail at the owning node. Bond order analysis remains unavailable.
- Coordination, cluster, RDF, histogram, reduce-property, and scatter-plot entries execute as pipeline modifiers and publish particle properties, global values, or data tables. Analysis tables are virtualized and export to CSV. Particle-table manual selection is one persistent pipeline node: click replaces the set, Ctrl-click toggles one particle, and invalid indices are reported rather than silently applied.
- Histogram, reduce-property, and color-coding property selectors now derive their choices from the selected node's enabled upstream property schema. Computed/CNA/coordination/cluster/color-coding outputs become selectable only downstream of their producer; removed properties and disabled producers are excluded, and downstream-only properties no longer leak into earlier nodes' selectors.
...
- Pipeline evaluation now takes ownership of its working `Dataset` snapshot, removing the extra full-dataset copy between the asynchronous worker input and its result. The application still makes one working copy from the retained source. It now keeps at most one estimated-64-MiB prefix snapshot immediately before the selected modifier; parameter edits to that modifier can resume from the prefix, while edits/removals/reorders before it, undo/redo, reset, and source loads invalidate it. Over-budget prefixes skip caching and run from source. This is a bounded single-prefix optimization, not general per-node caching or shared immutable storage.
- The execution loop reads immutable modifier parameters by reference instead of deep-copying each node on every evaluation; this avoids repeated copies of large selection vectors and type-pair cutoff tables. Prefix snapshots and the worker's private dataset remain separate copies.
...
- The export workflow integration test runs a real `App`/ImGui frame loop, records stable control IDs and rectangles, clicks Add modification and the RDF catalog entry, edits RDF parameters by keyboard, and verifies the recomputed table. It also uses visible Undo/Redo, selects and copies the node, changes only the copy's bin count, disables/re-enables it, moves it up/down in the visible stack, and deletes it while checking independent parameters and ordered outputs. Recorded interactive controls must fit inside the application bounds; this caught and fixed Pipeline action buttons clipped beyond the stack child, reversed visual move directions, and an oversized RDF input. An asynchronous failing node is also verified to retain the last result as stale, report the error on the owning node, and stop before an invalid downstream node. This tests ImGui widget/event behavior without claiming Windows-level desktop input or accessibility-tree coverage.
...
| Coordination analysis | 部分 | 可组合管线节点：Coordination 粒子属性、全局均值/邻居对数及配位数分布表；正交与三斜周期最小镜像；可按当前选区限制中心粒子和邻居，未选中粒子的 Coordination 为 0，均值与分布总体只包含选中粒子；不超过 200 万原子；超出精确空间索引比值范围的周期盒明确报错 |
| Radial distribution function (RDF) | 部分 | 周期 3D 粒子对直方图，正交或三斜晶胞体积归一化；管线节点可配置 1–4096 bins，默认 128；可按当前选区限制中心粒子和邻居，并按选中粒子数计算密度；限制每帧 200 万粒子和有限空间索引范围；二维 slab 不伪装为 3D RDF |
...
邻域分析与选择共享 linked-cell 邻居搜索内核；在采样数据上明确拒绝运行。截断邻居会损坏配位数与聚类结果，不能以可视化采样代替全数据科学分析。正交和三斜周期晶胞均走周期最小镜像；非正交最小镜像使用有界精确搜索，部分周期 slab/wire 只要求周期向量独立，缺失的非周期向量由搜索内部补基，不改变真实笛卡尔距离。200 万原子与候选比较上限用于防止超大任务无界运行，原子数预检先于邻域／CNA 工作数组分配。
...
| 粒子形状与类型外观 | 已实现 | 全局及逐类型显示/颜色/半径/形状覆盖；7 种形状（含椭球、方形和共享 OBJ 网格）及轴向比例可调。类型外观经 GPU 逐类型渲染回归验证；选择高亮与 Color coding 按既定优先级覆盖类型颜色，OBJ 网格目前为所有类型共享 |
| Color coding | 部分 | GPU 按节点配置的 Position、标量属性或向量 X/Y/Z 分量着色，向量分量也支持当前帧/全帧范围统计；另支持自动/对称/手动范围、离散、反转、仅选中和 Keep selection；全帧计算限每帧 200 万原子；视口、图例和 PNG 共用 256 项渐变表；拒绝全非有限值、GPU 无法表示的 double 值、非法范围及未知渐变。D3D11 端点钳制已通过 `-FLT_MAX..+FLT_MAX` GPU 回归；仍有全帧上限及渐变编辑等限制 |
| Affine transformation | 部分 | 已提供完整可编辑 3x4 矩阵；同步变换粒子、晶胞向量和原点并拒绝奇异矩阵；可选将 3x3 线性部分应用于向量属性（不应用平移）；法向量逆转置仍需按属性语义单独处理 |
