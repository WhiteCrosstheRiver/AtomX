...
| Color coding | 部分 | GPU 按节点配置的 Position、标量属性或向量 X/Y/Z 分量着色，向量分量也支持当前帧/全帧范围统计；另支持自动/对称/手动范围、离散、反转、仅选中和 Keep selection；全帧计算限每帧 200 万原子；视口、图例和 PNG 共用 256 项渐变表；拒绝全非有限值、GPU 无法表示的 double 值、非法范围及未知渐变。极宽 float 范围仍需修复和验收 |
