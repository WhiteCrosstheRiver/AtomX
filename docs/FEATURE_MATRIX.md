...
| Histogram | 部分 | 可选择位置分量或现有标量属性，配置 1–4096 bins 和绝对计数/相对频率/概率密度归一化，可限制为输入选区；发布含样本数的 Data Table；极端有限值范围使用缩放分箱避免跨度溢出；忽略非有限样本，拒绝缺失/错位属性和空有效样本，扫描和分箱可取消；直接读上游属性而不复制整列 |
